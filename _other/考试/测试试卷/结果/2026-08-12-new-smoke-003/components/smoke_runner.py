...
def audit_data(qid: str, audit_id: str) -> dict:
    ...
    # 候选范围的目标图校验可先发生，但 restricted_vector 才是偏好题的最终检索路线。
    if any(item.get("stage") == "restricted_vector" for item in events):
        route = "restricted_vector"
    elif any(item.get("status") == "not_found" for item in graph_events):
        ...
        if qid == "S07-A-01":
            for term in ("低脂", "低盐", "低热量"):
                for match in re.finditer(term, answer):
                    ...
                    if not any(token in window for token in ("不能", "不可", "没有", "未", "不应", "无法", "并非", "不是")):
                        strict += 1
    elif qid == "S08-A-01":
