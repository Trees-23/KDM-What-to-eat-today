# New 路径十题定向测试预检

- 运行编号：`2026-08-12-new-smoke-002`
- 实现提交：`2aa53400ca000779ae7c1b4372073fb66beda6ed`
- 当前分支：`fix/new-smoke-routing`
- 题库 SHA-256：`a2f0f729e41aa861fe5ca0c42e34c687894444f5873ca6e2673491b6bf328f72`
- 预检时间：`2026-08-12T13:42:27+08:00`

## 服务状态

- /health：`{"service":"RAG System","status":"healthy","timestamp":"2026-08-12 05:42:26.335748"}`
```text
what-to-eat-backend|running|Up 2 minutes (healthy)
what-to-eat-milvus-etcd|running|Up About an hour (healthy)
what-to-eat-milvus-minio|running|Up About an hour (healthy)
what-to-eat-milvus-standalone|running|Up About an hour (healthy)
what-to-eat-neo4j|running|Up About an hour (healthy)
```

## 原始预检 JSON

```json
{
  "artifact": {
    "manifest": {
      "created_at": "2026-08-11T13:45:23.382509+00:00",
      "milvus_build_id": "pds_2a8c0807733eb8022a623659",
      "milvus_collection": "cooking_knowledge_v2_pds_2a8c0807",
      "milvus_database": "default",
      "milvus_schema_hash": "bb34a179dcd8c4646cc0b2c416e1c6dfbbf8746f302f2161f896160987084e04",
      "pds_build_id": "pds_2a8c0807733eb8022a623659",
      "pds_manifest_sha256": "5644bef04a88bb8a3f99f526b394d88e0fdfc5d443d079e0b9a61dfd8cfbcec4",
      "rollback_collection": "cooking_knowledge_v2_pds_f01044e5",
      "rollback_database": "default",
      "rollback_pds_build": "pds_f01044e524ef43b413f76b02"
    },
    "milvus_row_count": 1333,
    "pds": {
      "anchor_count": 2634,
      "build_id": "pds_2a8c0807733eb8022a623659",
      "chunk_count": 1333,
      "parent_count": 341,
      "status": "ok"
    }
  },
  "bank_sha256": "e5edfbced75e84002bdd5b25054bf0906a022b4073e03ab31a4c85010f494f79",
  "failures": [],
  "health": {
    "service": "RAG System",
    "status": "healthy",
    "timestamp": "2026-08-12 05:42:19.000368"
  },
  "question_count": 300,
  "resolved_runtime_targets": 210,
  "status": "ready"
}
```

**预检通过，已在首次 API 请求前关闭 gold_manifest.json。**
