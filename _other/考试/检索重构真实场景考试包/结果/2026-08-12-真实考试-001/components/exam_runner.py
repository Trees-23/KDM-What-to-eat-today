...
def recipe_by_name(nodes: list[dict[str, str]], name: str) -> dict[str, str]:
    choices = [item for item in nodes if item.get("labels") == "Recipe" and item.get("name") == name]
    if not choices:
        raise RuntimeError(f"受治理 Recipe 源节点中不存在冻结候选：{name}")
    return sorted(choices, key=lambda item: str(item["nodeId"]))[0]
...
def semantic_items(question: dict[str, Any], nodes: list[dict[str, str]]) -> list[dict[str, Any]]:
    bank = load_bank()
    ...
    for relevance, title in zip((3, 2, 1), titles):
        parent = recipe_by_name(nodes, title)
        items.append({
            "key": parent["nodeId"], "name": parent["name"], "relevance": relevance,
            "sourcePath": normalize_source_path(parent.get("filePath", "")),
            "reason": f"冻结于受治理源 Markdown 的菜名、分类/菜系和做法信息；与题干意图的直接程度为 {relevance}。",
        })
    return items
...
def make_manifest() -> None:
    ...
    for question in bank["questions"]:
        ...
        if sid in {"S01", "S02", "S03"}:
                ...
                for rel in requires:
                    ...
                    if sid == "S04":
                        ...
                        for other in requires:
                            ...
                            if vegetable and vegetable.get("labels") == "Ingredient" and (other.get("ingredientCategory") or vegetable.get("category")) == "蔬菜":
            ...
            expected_zero_paths = sid == "S05" and target.get("expected_verified_graph_paths") == 0
            if expected_zero_paths and graphs:
                raise RuntimeError(f"{question['question_id']} 预期零路径但冻结到 {len(graphs)} 条图路径")
            if not expected_zero_paths and not graphs:
            ...
            if expected_zero_paths:
                record["zero_path_evidence"] = {
                    "count": 0,
                    "query": "MATCH (ingredient:Ingredient)<-[:REQUIRES]-(recipe:Recipe)-[r:REQUIRES]->(vegetable:Ingredient) WHERE ingredient.nodeId = $ingredient_id AND vegetable.nodeId <> ingredient.nodeId AND coalesce(r.ingredientCategory, vegetable.category) = '蔬菜' RETURN count(DISTINCT [recipe.nodeId, vegetable.nodeId])",
                    "parameters": {"ingredient_id": record["entity_resolution"][0]["key"]},
                    "source": "preflight-probe.json（稳定 nodeId 的真实 Neo4j 只读核验）",
                }
        elif sid in {"S06", "S07"}:
            record["gold_items"] = semantic_items(question, nodes)
    ...
    manifest = {
        ...
        "source_of_truth": {"nodes": "data/cypher/nodes.csv + data/cypher/tips_nodes.csv，经 preflight-probe.json 的真实 Neo4j nodeId/sourcePath 只读核验", "relations": "data/cypher/relationships.csv，经 preflight-probe.json 的真实 Neo4j 图路径只读核验", "pds": str(PDS_PATH.relative_to(ROOT))},
...
def write_preflight() -> None:
    """将只读检查的原始 JSON 与固定运行元数据写入本运行目录。"""
    probe_path = OUT / "preflight-probe.json"
    probe = json.loads(probe_path.read_text(encoding="utf-8"))
    bank_sha = hashlib.sha256(BANK_PATH.read_bytes()).hexdigest()
    report_sha = re.search(r"题库 SHA-256：`([0-9a-f]{64})`", (ROOT / "_other/考试/题库校验报告.md").read_text(encoding="utf-8"))
    ...
    health = json.dumps(probe.get("health"), ensure_ascii=False)
    lines = [
        "# 真实服务考试预检", "",
        f"- 运行编号：`{RUN_ID}`",
        f"- 预检关闭时间：`{datetime.now(timezone.utc).astimezone().isoformat(timespec='seconds')}`",
        f"- 实现提交：`{command('git', 'rev-parse', 'HEAD')}`",
        f"- 当前分支：`{command('git', 'branch', '--show-current')}`",
        f"- 目标基线：`origin/main` 的 `{command('git', 'rev-parse', 'origin/main')}`",
        f"- 题库 SHA-256：`{bank_sha}`",
        f"- `题库校验报告.md` 声明的 SHA-256：`{report_sha.group(1) if report_sha else '未解析'}`",
        ...
        "", "## 只读服务状态", "",
        f"- `GET /health`：`{health}`",
        f"- 解析的运行时目标数：`{probe.get('resolved_runtime_targets')}`。",
        f"- PDS：parents=`{probe['artifact']['pds']['parent_count']}`，chunks=`{probe['artifact']['pds']['chunk_count']}`；Milvus V2 行数=`{probe['artifact']['milvus_row_count']}`。",
        "- Compose 容器状态：", "```text", containers, "```", "",
        "## 原始预检 JSON", "", "```json", json.dumps(probe, ensure_ascii=False, indent=2, sort_keys=True), "```", "",
        "## 预检结论", "", "**通过，已在任何 API 请求前关闭 gold_manifest.json。**",
    ]
...
def audit_path(variant: str, qid: str, audit_id: str) -> dict[str, Any]:
    ...
    graph_statuses = [str(value.get("status")) for value in events if value.get("stage") in {"targeted_graph", "targeted_graph_selection"}]
    if "not_found" in graph_statuses: observed = "graph_not_found"
    elif any(value.get("stage") == "targeted_graph_selection" or value.get("stage") == "targeted_graph" for value in events): observed = "targeted_graph"
    ...
    return {
        "route": observed, "graph_statuses": graph_statuses, "events": events, "entity_resolution": entities,
...
def checks_for(question: dict[str, Any], answer: str, audit: dict[str, Any] | None, variant: str, status: str) -> dict[str, Any]:
    ...
    route_ok = observed != "legacy_hybrid" if variant == "new" else observed == "legacy_hybrid"
    if sid == "S08":
    ...
    if sid == "S05" and question["difficulty_code"] == "C":
        declined = any(word in answer for word in ("无法证明", "不能证明", "没有路径", "未找到", "不存在"))
        unsupported = 0 if declined else int(bool(answer.strip()))
        return {"route_correct": bool(audit and observed == "graph_not_found"), "evidence_complete": bool(audit and observed == "graph_not_found"), "evidence_linked": bool(audit and observed == "graph_not_found"), "answer_faithful": declined and unsupported == 0, "safety_pass": declined and unsupported == 0, "forbidden_assertion_count": unsupported, "unsupported_relation_claim_count": unsupported, "strict_nutrition_misreport_count": 0}
    strict = 0
    if sid == "S07":
...
def run_api(variant: str) -> None:
    ...
    for number, question in enumerate(api_questions, start=1):
        ...
        elif question["scenario_id"] == "S05" and question["difficulty_code"] == "C" and audit and audit["route"] != "graph_not_found":
            status = "blocked"
            error_reason = f"S05 C 审计未证明零路径，观测路由为 {audit['route']}"
        ...
        ranking = [] if question["scenario_id"] == "S05" and question["difficulty_code"] == "C" else (audit["ranking"] if audit else [])
        gold_items = [] if question["scenario_id"] == "S05" and question["difficulty_code"] == "C" else gold[qid].get("gold_items", [])
        rows.append({"question_id": qid, "scenario_id": question["scenario_id"], "difficulty_code": question["difficulty_code"], "variant": variant, "evaluation_mode": question["contract"]["evaluation_mode"], "status": status, "audit_id": audit_id, "route": route, "ranking": ranking, "gold_items": gold_items, "path": path, "checks": checks, "timing": {"ttft_ms": audit["ttft"] if audit else None, "total_latency_ms": audit["total"] if audit else elapsed}, "answer": answer, "exam_note": error_reason})
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=("prepare", "freeze", "api", "components", "blocked"))
    parser.add_argument("--variant", choices=("old", "new"))
    args = parser.parse_args()
    if args.mode == "prepare":
        write_preflight()
        make_manifest()
    elif args.mode == "freeze": make_manifest()
