...
这是持续执行委托。你必须从阶段 0 开始，在每个阶段 DoD 通过后立即推进下一阶段，持续完成实现、验证、提交、推送、PR 维护、staging 验收和最终场景验证，直至阶段 6 DoD 完成；不能把提交计划、单个模块、一次测试或 PR 更新当作终点。你不需要为常规实现选择、测试修复、依赖处理、提交、推送、PR 更新、staging 验收或回退演练向用户询问，但绝不跳过门槛，也不得把未经证实的 schema、CSV 或语义相似度当作事实。

成功终点是：实施方案阶段 6 的 DoD 已满足，十个固定用户场景、实体不存在、图关系不存在、图服务失败三类降级、离线评测阈值、staging 用户场景验收、feature flag 回退和所需备份恢复演练均有可审计证据。main 合并不属于本委托的终点，仍需用户明确确认。
...
发生冲突时，优先级依次为：用户最新指令、AGENTS.md、实施方案的阶段门槛、V2、当前代码。若运行实例的只读事实与实施方案/V2 的稳定 ID、关系方向、字段、分类或迁移前提冲突，记录精确差异并保持受影响阶段阻塞；等待用户或方案维护者已发布的明确方案修订后才能继续，不为此自行发起常规决策请求。运行实例事实不能仅靠 PR 注释授权执行代理自行改写阶段契约、schema、Cypher 模板或用户可见行为。
...
1. 把全部阶段视为一个连续目标。当前阶段一旦满足 DoD，立即执行下一阶段，不等待“继续”“批准下一阶段”或对常规技术细节的答复。
2. 对测试失败、lint/类型错误、依赖缺失、接口不一致、迁移脚本缺陷、staging 验收失败，先自行定位根因，做最小修复，增加回归测试，重复验证；不得以“需要用户决定”为由跳过、降级或提前结束。
3. 主动寻找已存在且已授权的输入：仓库配置、受保护 CI/CD 变量、阶段基线记录、部署文档、审批记录、已运行服务和 staging 环境。不得猜测凭据、目标实例、审批或数据事实。
4. 每个阶段的“审核通过”都由独立审查代理完成：该代理不继承实现上下文、不改工作区，只审查阶段 diff、测试/服务证据、实施方案章节和 PR。将其身份、输入 commit、结论、发现项和时间写入 docs/retrieval/reviews/phase-<n>-<commit>.md；不通过则自行修复并重审。干净测试进程只能提供自动化辅助证据，不能替代独立审查签收或最终用户场景签收。
5. 只有不可由本地实现解决的外部状态才允许阻塞：没有已授权的真实服务/数据目标、实例事实与冻结契约实质冲突、受保护审批记录缺失、或营养数据治理决策不存在。遇到这些情况，完成所有安全的本地/staging 工作、提交诊断和已通过证据，持续用安全只读检查或可用监控机制观察状态；不要反复向用户提问，也不要用模拟结果、未批准的导入或破坏性命令伪造通过。对于阶段 6 的 7 天观察窗口，先部署实施方案规定的每 15 分钟受保护 rollout 监控并让它持续采样；缺少指标源/调度器或到第 8 天仍不足 100 请求时，生成 blocked 报告、保持旧流量并继续采样，不能将等待期视为通过。
6. 每一个阶段都必须完整执行实施方案规定的单元测试、集成/真实服务检查、用户场景验收、feature flag 回退和提交/PR 更新。不能用“后续补测试”“先重构再调试”或一次性切换替代。
...
4. A2 只能在已经存在并可核验的授权目标标识后执行。主动检查受保护部署配置、阶段基线记录、运行环境和已有审批记录中是否已有：Neo4j 的非敏感 URI/实例标识、database、允许只读查询；Milvus 的 host:port/实例标识、database、collection、允许只读查询。绝不从本地默认配置猜测目标，绝不把密码、token、完整连接串或查询返回的敏感字段写入记录。目标确认后才安装/使用所需客户端并执行：
...
5. B 是 CSV 导入可用性门槛，仅在 A2 的 Neo4j 目标确认后执行。先通过 A2 已确认的容器/服务器只读入口检查 Neo4j 实际 import 目录中 nodes.csv 可读；再在同一 database 执行下列只读预检，成功返回一行才算 B 通过：
...
若 B 失败，先完成阶段 0 诊断、建议的最小修正和静态验证方案；不得改 docker-compose.yml、挂载、导入脚本、启动命令或任何数据，除非实施环境已有对应受保护审批记录。即便得到该记录，也只执行该记录明确允许的静态/隔离修正，不导入、不重建数据。
...
如果 A2 因没有安全的运行实例、凭据、镜像或已存在的授权目标而无法完成，保持阶段 0 为 blocked。完成所有安全只读检查、回归修复、基线记录和已完成诊断后，继续通过可用监控/重试机制检查是否出现已授权目标；不要向用户重复询问。不得改用模拟结果、导入脚本或自行创建空数据库来宣称通过，也不得开始阶段 1。
...
5. 需要 Neo4j/Milvus 的阶段，完成真实服务检查、样本核对和独立用户场景验收。
...
发生测试失败、服务不可用、schema 与预期不一致、数据治理缺失、白名单不全或用户验收不通过时：不跨阶段；保留可诊断证据；优先自行修复和重跑不依赖阻塞项的当前阶段安全工作。只有达到“自主持续推进规则”中的不可解外部状态时才维持 blocked，并持续安全检查；不得因普通失败停止或向用户索取常规决策。
...
- 进入前必须存在可审计的产品/数据治理出口：严格营养出口冻结“低脂”的来源、单位、每份定义、阈值、审核、版本、过期和医疗边界；或软偏好出口明确没有可信来源、严格模式保持关闭及医疗/阈值请求返回证据不足。
...
- 部署并验证 scripts/monitor_retrieval_rollout.py 与受保护的 15 分钟 CI 调度；只有 rollout-window.json 满足连续 7 天、100 请求和全部冻结阈值时才扩大流量。第 8 天仍未达样本量或指标不可用时写 blocked 报告、保持旧流量并继续采样。
- 主动由可识别的独立审查代理或非实施者在 staging UI/API 做最终用户验收并签收；干净测试进程仅可作为辅助证据。旧路径保留在 RETRIEVAL_LEGACY_FALLBACK_ENABLED 下，直到单独批准删除。
...
- rollout 监控身份、指标源别名、artifact、rollout-window 结论和下一次计划运行时间；
- feature flag、当前值、回退步骤和任何未演练项；
- 阻塞项、已完成的自主修复、持续检查方式，以及尚缺的外部环境、数据治理决策或受保护审批记录；
