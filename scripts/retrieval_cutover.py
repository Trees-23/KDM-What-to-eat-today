...
PERSONAL_LOCAL_PROFILE = "personal-local"
PROTECTED_PROFILE = "protected"
PERSONAL_LOCAL_ENVIRONMENT = "personal-local"
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--release-profile", choices=(PROTECTED_PROFILE, PERSONAL_LOCAL_PROFILE), default=PROTECTED_PROFILE)
    ...
    parser.add_argument("--allowed-approval-root")
...
def _validate(args: argparse.Namespace) -> RetrievalArtifactManifest:
    ...
    if args.release_profile == PROTECTED_PROFILE:
        if not args.protected_environment or not args.approval_record or not args.allowed_approval_root:
            raise CutoverGuardError("受保护切换必须提供环境和审批记录")
    ...
    elif args.environment != PERSONAL_LOCAL_ENVIRONMENT or runtime_environment != PERSONAL_LOCAL_ENVIRONMENT:
        raise CutoverGuardError("personal-local 仅允许显式绑定本机个人环境")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"status": "cutover", "profile": args.release_profile, "database": args.database, "collection": args.to_collection, "rollback_collection": manifest.rollback_collection}, ensure_ascii=False))
    return 0
