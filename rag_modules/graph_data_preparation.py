...
class GraphDataPreparationModule:
    ...
    def __init__(self, uri: str, user: str, password: str, database: str = "neo4j"):
        ...
        self.technique_docs: List[GraphNode] = []
        self.technique_chunks: List[GraphNode] = []
    ...
    def load_graph_data(self) -> Dict[str, Any]:
        ...
        with self.driver.session() as session:
            ...
            # 加载技巧/知识文档节点
            technique_docs_query = """
            MATCH (t:TechniqueDoc)
            RETURN t.nodeId as nodeId, labels(t) as labels, t.name as name,
                   properties(t) as properties
            ORDER BY t.nodeId
            """

            result = session.run(technique_docs_query)
            self.technique_docs = []
            for record in result:
                node = GraphNode(
                    node_id=record["nodeId"],
                ...
                self.technique_docs.append(node)

            logger.info(f"加载了 {len(self.technique_docs)} 个技巧文档节点")

            # 加载技巧/知识文档分块节点
            technique_chunks_query = """
            MATCH (c:TechniqueChunk)
            RETURN c.nodeId as nodeId, labels(c) as labels, c.name as name,
                   properties(c) as properties
            ORDER BY c.nodeId
            """

            result = session.run(technique_chunks_query)
            self.technique_chunks = []
            for record in result:
                node = GraphNode(
                    node_id=record["nodeId"],
                    labels=record["labels"],
                ...
                self.technique_chunks.append(node)

            logger.info(f"加载了 {len(self.technique_chunks)} 个技巧文档分块节点")
        
        return {
            'recipes': len(self.recipes),
            'ingredients': len(self.ingredients),
            'cooking_steps': len(self.cooking_steps),
            'technique_docs': len(self.technique_docs),
            'technique_chunks': len(self.technique_chunks)
        }
    ...
    def build_technique_documents(self) -> List[Document]:
        """
        构建技巧/知识文档，来源于 Neo4j 中的 TechniqueDoc / TechniqueChunk。
        """
        logger.info("正在构建技巧/知识文档...")
        documents = []

        with self.driver.session() as session:
            docs_query = """
            MATCH (t:TechniqueDoc)
            OPTIONAL MATCH (t)-[r:HAS_CHUNK]->(c:TechniqueChunk)
            WITH t, c, r
            ORDER BY t.nodeId, COALESCE(r.chunkOrder, c.chunkIndex, 999)
            WITH t, collect(c) as chunks
            RETURN t.nodeId as nodeId,
                   t.name as name,
                   t.title as title,
            ...
            for record in result:
                try:
                    node_id = record["nodeId"]
                    title = record["title"] or record["name"] or node_id
                    category = record["category"] or "烹饪技巧"
                    source_path = record["sourcePath"] or ""
                    tags = record["tags"] or ""
                    ...
                    chunks = [chunk for chunk in (record["chunks"] or []) if chunk]
                    if chunks:
                        for chunk in chunks:
                            props = dict(chunk)
                            section_title = props.get("sectionTitle") or props.get("name") or "正文"
                            chunk_content = props.get("content") or props.get("summary") or ""
                            if chunk_content:
                                content_parts.append(f"\n## {section_title}\n{chunk_content}")
                    elif record["content"]:
                        content_parts.append(f"\n## 正文\n{record['content']}")
                    ...
                    logger.warning(f"构建技巧文档失败 {record.get('name', '')}: {e}")
                    continue

        self.documents.extend(documents)
        logger.info(f"成功构建 {len(documents)} 个技巧/知识文档")
        return documents
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_technique_docs': len(self.technique_docs),
            'total_technique_chunks': len(self.technique_chunks),
