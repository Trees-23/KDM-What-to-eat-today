...
class GraphIndexingModule:
    ...
    def create_entity_key_values(
        ...
        technique_docs: Optional[List[Any]] = None,
        technique_chunks: Optional[List[Any]] = None,
        ...
        # 处理技巧/知识文档实体
        for technique_doc in technique_docs or []:
            entity_id = technique_doc.node_id
            entity_name = technique_doc.name or f"技巧文档_{entity_id}"

            content_parts = [f"技巧文档: {entity_name}"]
            index_keys = [entity_name]

            if hasattr(technique_doc, 'properties'):
                props = technique_doc.properties
                if props.get('title') and props.get('title') not in index_keys:
                    index_keys.append(props['title'])
                if props.get('category'):
                    content_parts.append(f"分类: {props['category']}")
                    index_keys.append(props['category'])
                if props.get('tags'):
                    content_parts.append(f"标签: {props['tags']}")
                    index_keys.extend([tag.strip() for tag in str(props['tags']).split(',') if tag.strip()])
                if props.get('summary'):
                    content_parts.append(f"摘要: {props['summary']}")
                if props.get('sourcePath'):
                    content_parts.append(f"来源: {props['sourcePath']}")
            ...
            entity_kv = EntityKeyValue(
                ...
                metadata={
                    "node_id": entity_id,
                    "properties": getattr(technique_doc, 'properties', {})
                }
            )
        ...
        # 处理技巧/知识文档章节实体
        for technique_chunk in technique_chunks or []:
            entity_id = technique_chunk.node_id
            entity_name = technique_chunk.name or f"技巧章节_{entity_id}"

            content_parts = [f"技巧章节: {entity_name}"]
            index_keys = [entity_name]

            if hasattr(technique_chunk, 'properties'):
                props = technique_chunk.properties
                for key_name in ("title", "sectionTitle", "category"):
                    value = props.get(key_name)
                    if value:
                        index_keys.append(value)
                if props.get('tags'):
                    index_keys.extend([tag.strip() for tag in str(props['tags']).split(',') if tag.strip()])
                if props.get('sectionTitle'):
                    content_parts.append(f"章节: {props['sectionTitle']}")
                if props.get('category'):
                    content_parts.append(f"分类: {props['category']}")
                if props.get('summary'):
                    content_parts.append(f"摘要: {props['summary']}")
                if props.get('content'):
                    content_parts.append(f"内容: {props['content'][:1200]}")

            entity_kv = EntityKeyValue(
                entity_name=entity_name,
                index_keys=list(dict.fromkeys(index_keys)),
                value_content='\n'.join(content_parts),
                entity_type="TechniqueChunk",
                metadata={
                    "node_id": entity_id,
                    "properties": getattr(technique_chunk, 'properties', {})
                }
            )
